...
class MistralAIClient:
    ...
    def create(self, params: dict[str, Any]) -> ChatCompletion:
        ...
        if "messages" in mistral_params:
            # import pprint
            # pprint.pprint(params["messages"])

            mistral_params["messages"] = patch_mistral_messages(mistral_params["messages"])
        ...
        # Hannah - trying to extract valid text chunk
        raw_content = mistral_response.choices[0].message.content
        # print("[Mistral] Raw assistant content:", raw_content)
...
'''
    Hannah - Trying to fix invalid message order for Mistral
'''
def patch_mistral_messages(messages):
    #print("[Patch] Scanning full message list")
    patched = []
    tool_call_ids = set()
    tool_response_ids = set()
    assistant_tool_calls = []

    # Sanitize all tool call IDs 
    for m in messages:
        if isinstance(m, AssistantMessage) and hasattr(m, "tool_calls") and m.tool_calls:
            for tc in m.tool_calls:
                original_id = tc.id
                sanitized_id = sanitize_tool_call_id(original_id)
                tc.id = sanitized_id
                #print(f"[Patch] Sanitized tool call ID {original_id} -> {sanitized_id}")


    # Collect all tool call IDs from AssistantMessages
    for m in messages:
        if isinstance(m, AssistantMessage) and hasattr(m, "tool_calls") and m.tool_calls:
            for tc in m.tool_calls:
                tool_call_ids.add(tc.id)
                assistant_tool_calls.append(tc.id)
                #print(f"[Patch] Found tool call with id {tc.id}")

    #  Process messages in order, ensuring proper sequence 
    i = 0
    while i < len(messages):
        ...
        if isinstance(m, AssistantMessage) and hasattr(m, "tool_calls") and m.tool_calls:
            ...
            # Ensure next messages are ToolMessages for each tool call
            for tc in m.tool_calls:
                ...
                for j in range(i, len(messages)):
                    if (isinstance(messages[j], ToolMessage) and 
                        ...
                        tool_response_ids.add(messages[j].tool_call_id)
                        i = j + 1
                        found = True
                        break
                
                # If no matching ToolMessage found, add a dummy one
                if not found:
                    #print(f"[Patch] Adding dummy ToolMessage for tool_call_id {tc.id}")
                    patched.append(ToolMessage(
                        name=tc.function.name,
                        content="{}",
                        tool_call_id=tc.id
                    ))
                    tool_response_ids.add(tc.id)
            
            # After tool responses, ensure there's an AssistantMessage
            if i < len(messages) and not isinstance(messages[i], AssistantMessage):
                #print("[Patch] Adding acknowledgment AssistantMessage")
                patched.append(AssistantMessage(content="Acknowledged."))
        
        # Handle other message types
        else:
            # Skip duplicate ToolMessages
            if isinstance(m, ToolMessage):
                if m.tool_call_id in tool_response_ids:
                    #print(f"[Patch] Skipping duplicate ToolMessage with id {m.tool_call_id}")
                    i += 1
                    continue
    ...
    # Add dummy ToolMessages for any unresponded tool calls 
    for tc_id in tool_call_ids:
        if tc_id not in tool_response_ids:
            #print(f"[Patch] Adding dummy ToolMessage for missing tool_call_id {tc_id}")
            patched.append(ToolMessage(
                name="dummy_tool",
                content="{}",
                tool_call_id=tc_id
    ...
    #  Ensure the final message is a UserMessage or ToolMessage
    if len(patched) > 0 and not isinstance(patched[-1], (UserMessage, ToolMessage)):
        print("[Patch] Final message is not UserMessage or ToolMessage --> inserting dummy UserMessage")
        patched.append(UserMessage(content="Continue."))

    # Debug print the full message sequence 
    
    #print("[Patch] Final message sequence:")
    for i, m in enumerate(patched):
        ...
        '''
        print(f"{i}: {role} | {type(m).__name__} | ", end="")
        if tool_calls:
            print(f"tool_calls={[tc.id for tc in tool_calls]}")
        elif tool_call_id:
