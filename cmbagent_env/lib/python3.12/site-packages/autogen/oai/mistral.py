...
def patch_mistral_messages(messages):
    ...
    while i < len(messages) - 1:
        ...
        # ToolMessage not followed by AssistantMessage
        if isinstance(m, ToolMessage) and not isinstance(next_m, (AssistantMessage)):
            print(f"[Patch] ToolMessage at index {i} not followed by AssistantMessage --> inserting acknowledgment")
            messages.insert(i + 1, AssistantMessage(content="Acknowledged tool result."))
            i += 2  # skip the new AssistantMessage
        else:
            i += 1

    # Fix: SystemMessages not at index 0 --> convert to UserMessages
    for i in range(1, len(messages)):
        if isinstance(messages[i], SystemMessage):
            print(f"[Patch] SystemMessage at index {i} is not first --> converting to UserMessage")
            messages[i] = UserMessage(content=messages[i].content)
    ...
    if isinstance(last_msg, AssistantMessage):
        if not getattr(last_msg, "tool_calls", None):
            print("[Patch] Final AssistantMessage without tool_calls --> appending dummy UserMessage")
            messages.append(UserMessage(content="OK"))

    elif isinstance(last_msg, ToolMessage):
        print("[Patch] Final message is ToolMessage --> inserting AssistantMessage + dummy UserMessage")
        messages.append(AssistantMessage(content="Acknowledged tool result."))
        messages.append(UserMessage(content="OK"))

    print("[Patch] Final message sequence:")
    for i, m in enumerate(messages):
        print(f"{i}: {m.role} | {type(m).__name__}")
    ...
    for msg in messages:
        if isinstance(msg, AssistantMessage) and msg.tool_calls:
            for tool_call in msg.tool_calls:
                old_id = tool_call.id
                new_id = sanitize_tool_call_id(old_id)
                if old_id != new_id:
                    print(f"[Patch] ToolCall ID changed: {old_id} --> {new_id}")
                    tool_call.id = new_id
                    tool_call_id_map[old_id] = new_id

    for msg in messages:
        if isinstance(msg, ToolMessage) and msg.tool_call_id in tool_call_id_map:
            msg.tool_call_id = tool_call_id_map[msg.tool_call_id]
