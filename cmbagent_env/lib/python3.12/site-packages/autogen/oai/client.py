...
if openai_result.is_successful:
    ...
    from openai import InternalServerError as openai_InternalServerError # Hannah added
...
if together_result.is_successful:
    from together.error import InvalidRequestError as together_invalidRequestError # Hannah added
...
if groq_result.is_successful:
    from groq import APIStatusError as groq_APIStatusError # Hannah added
...
class OpenAIClient:
    ...
    def message_retrieval(
        ...
        # Hannah - TODO handle Llama weird responses
    ...
    def create(self, **config: Any) -> ModelClient.ModelClientResponseProtocol:
        ...
        for i, client in enumerate(self._clients):
            ...
            try:
                ...
                # Hannah - TODO switch to different model if one fails
                try:
                    request_ts = get_current_ts()
                    response = client.create(params)
                except groq_APIStatusError as e:
                    #   Groq request too large
                    if e.status_code == 413 and i != last:
                        print(f"[Fallback] Skipping Groq model due to 413 error: {e}")
                        continue
                    # Groq: Bad request error
                    elif e.status_code == 400 and i != last:
                        print(f"[Fallback] Skipping Groq model due to 400 BadRequestError: {e}")
                        continue
                    else:
                        raise  # Re-raise if not handled

                except mistral_SDKError as e:
                    if e.status_code == 500 and i != last:
                        print(f"[Fallback] Skipping Mistral model due to 500 error: {e}")
                        continue
                    elif e.status_code == 400 and i != last:
                        print(f"[Fallback] Skipping Mistral model due to 400 error: {e}")
                        continue  
                    elif e.status_code == 429 and i != last:
                        print(f"[Fallback] Skipping Mistral model due to 429 error: {e}")
                        continue  
                 
                    else:
                        raise

                except together_invalidRequestError as e:
                    if "400" in str(e) and i != last: 
                        print(f"[Fallback] Skipping Together model due to 400 error: {e}")
                        continue
                    elif "422" in str(e) and i != last:
                        print(f"[Fallback] Skipping Together model due to 422 error: {e}")
                    else:
                        raise

                except openai_InternalServerError as e:
                    if "500" in str(e) and i != last:
                        print(f"[Fallback] Skipping OpenAI model due to 500 InternalServerError: {e}")
                        continue
                    else:
                        raise
